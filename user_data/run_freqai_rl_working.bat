@echo off
rem =============================================================================
rem FreqAI强化学习交易机器人启动脚本 - 完全中文化版本
rem =============================================================================
rem
rem 功能说明:
rem 1. 自动激活虚拟环境
...
set LOG_FILE=user_data\logs\freqai_working_%RANDOM%.log
...
if exist "user_data\logs\*.log" (
    for /f %%i in ('dir "user_data\logs\*.log" /od /b 2^>nul ^| tail -1') do (
        echo 最新日志文件: %%i
        echo 最后几行内容:
        tail -5 "user_data\logs\%%i" 2>nul
    )
) else (
    echo 没有找到日志文件
)
...
echo [信息] 最近的日志文件:
dir "user_data\logs\*.log" /od /b 2>nul | tail -5
echo.

echo [信息] 显示最新日志文件的最后20行:
for /f %%i in ('dir "user_data\logs\*.log" /od /b 2^>nul ^| tail -1') do (
    echo ======== %%i ========
    tail -20 "user_data\logs\%%i" 2>nul
)
goto end
...
echo [检查3] 关键依赖包...
pip list | findstr "freqtrade stable-baselines3 gymnasium"
echo.
